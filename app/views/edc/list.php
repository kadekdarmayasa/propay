  ...
  <div class="table-container">
    ...
    <div class="header">
      <div class="left-header">
        ...
        <form action="" method="post">
          <?php if (isset($data['keyword']) && $data['keyword'] != '') : ?>
            <input type="text" name="edc-field" id="edc-field" placeholder="Search EDC..." value="<?= $data['keyword'] ?>" autocomplete="off">
          <?php else : ?>
            <input type="text" name="edc-field" id="edc-field" placeholder="Search EDC..." autocomplete="off">
          <?php endif; ?>
          <button type="submit" name="search-edc"><svg width="17" height="18" viewBox="0 0 17 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="right-header">
        ...
        <a href="<?= BASEURL . 'edc_list/add' ?>" class="add-class-btn">
    ...
    <!-- Classlist Data -->
    <div class="data">
      <?php if (count($data['edc']) == 0) : ?>
        <p class="empty-message">There is no EDC found</p>
      <?php else : ?>
        <table>
          <tr>
            <th>#</th>
          ...
          ?>
            <tr>
              ...
              <td>
                <?php if ($data['edc'][$i]['total_students'] == 0) : ?>
                  -
                <?php elseif ($data['edc'][$i]['total_students'] < 2) : ?>
                  <?= $data['edc'][$i]['total_students']; ?> person
                <?php else : ?>
                  <?= $data['edc'][$i]['total_students']; ?> people
                <?php endif; ?>
              </td>
              <td>
                <a href="<?= BASEURL . 'edc_list/update/' . $data['edc'][$i]['edc_id'] ?>" class="update-btn">
...
<over-lay href="<?= BASEURL . 'edc_list/delete_edc' ?>"></over-lay>